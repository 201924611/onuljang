// data.js — 도매시장·작목 마스터 데이터 + 결정론적 시드 가격 생성기
// ───────────────────────────────────────────────────────────────────────────
// ⚠ 데이터 출처/성격 안내(정직성):
//  - 시장 코드/명칭/위치, 품목 분류는 aT 공영도매시장·KAMIS 분류 체계와 호환되도록 구성한 마스터다.
//  - 가격(경락가)은 데모 안정성을 위해 "결정론적 시드"로 생성한 합성 시계열이다.
//    (헤드리스·무키 환경에서 실시간 API 키 없이도 재현 가능하게 하기 위함)
//  - 실제 운영 시 동일 스키마로 공공데이터포털 API를 그대로 대체 호출한다:
//      · 전국 공영도매시장 실시간 경매정보  data.go.kr/data/15141808 (경락가·거래량·등급·시장코드)
//      · 도매시장 경락가격 표준데이터        data.go.kr/data/15029181 (시계열 백필)
//      · 기상청 ASOS 일자료                  data.go.kr/data/15059093 (기상 외생변수)
//    -> src/liveAdapter.js 의 fetchLiveAuction() 자리에 ServiceKey만 넣으면 라이브 전환.
